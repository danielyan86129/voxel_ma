...
public:
    ...
    bool loadFromTetgenFiles(const tetgenio& _tetio,
                             shared_ptr<Volume3DScalar> _vol = nullptr);
    //
    // init from given information
    void setInfo(const vector<point>& _vts, const vector<ivec2>& _edges,
    ...
    // this version assumes sites for each face is correctly associated and only
    // computes vert-radius
    void computeInfoRelatedtoSites();
    // this version takes in the centroid for each cell and does 1-nn to
    // associate sites with each face then it also computes vert-radius
    ...
    // NOTE: radii only valid after they are initialized, e.g. by calling
    // setClosestSites()
    ...
    //
    // given indices, return actual geom representation (faces will be
    // triangulated)
    void getVts(const vector<int>& _vts_indices, vector<point>& _vts) const;
    void getEdges(const vector<int>& _edges_indices,
                  vector<ivec2>& _edges) const;
    void getFaces(const vector<int>& _faces_indices,
                  vector<uTriFace>& tri_faces) const;
    //
    // get the voro-diagram i.e. vts, edges, and (triangulated)faces, with the
    // invalid elements pruned.
    void getPartInside(vector<int>& _vts_indices, vector<ivec2>& _edges,
                       vector<uTriFace>& _tri_faces) const;
    //
    // return inside part (v/e/f) together with the specified measure (on v/e/f)
    void extractInsideWithMeasure(
    ...
    inline cellcomplex& geom() { return m_geom; }
    inline const cellcomplex& geom() const { return m_geom; }
    //
    ...
    // @param _only_inside: will only perform merging on inside elements & save
    // IVD
    void mergeCloseVts(float _eps, bool _only_inside);
    //
    // return the euler characteristic for this voro
    void computeEulerChar(eulerchar& _euler_struct,
                          bool _do_prune_first = true);
    //
    // compute the requested measure for vts/edges/faces
    void computeVertexMeasure(MeasureForMA::meassuretype _mssure_tp,
                              const vector<int>& _vts_indices,
                              vector<float>& _vts_msure) const;
    void computeEdgesMeasure(MeasureForMA::meassuretype _mssure_tp,
                             const vector<int>& _edges_indices,
                             vector<float>& _edges_msure) const;
    void computeFacesMeasure(MeasureForMA::meassuretype _mssure_tp,
    ...
    void computeEdgesMeasure(const vector<float>& _vts_msure,
                             const vector<int>& _edges_indices,
                             vector<float>& _edges_msure) const;
...
protected:
    ...
    void load_voro_edges(
        ifstream& _in_edge, shared_ptr<Volume3DScalar> _vol = nullptr,
        const unordered_map<int, int>* const _old_new_v_map = nullptr,
        unordered_map<int, int>* _old_new_e_map = nullptr);
    void load_voro_faces(
        ifstream& _in_face, shared_ptr<Volume3DScalar> _vol = nullptr,
    ...
    //
    // compute the multiplicity of each vertex, including the infinite vert
    // (ref. by edge/face as -1)
    ...
    */
    ...
    // multiplicity of each vert (how many surface components touch at vi
    // singularly)
    ...
    int m_only_inside;       // is there only inside part left in this voro? -1:
