...
struct SpaceConverter
{
	...
	static inline point getVoxelCorner( const ivec3& _vox, int _ix, int _iy, int _iz )
	{
	...
	static bool voxTaggedAsInside( const ivec3& _vox, const shared_ptr<Volume3DScalar> _vol)
	{
		return ( !SpaceConverter::outOfVolBounds( _vox, _vol ) ) &&
			( get_occupancy_at_vox( _vox, _vol ) == 1 );
	}
	// conversion from/to voxel space (continuous) to/from 3d model space (continuous)
	// (where both voxel and 3d geometry points are floating numbers)
	static inline void fromVoxToModel( const point& _v, point& _p, const xform& _vox2model )
	{
		//_p = _v + point( 0.5f );
		_p = _v;
		_p = _vox2model * _p;
	...
	static inline void fromModelToVox( const point& _p, point& _q, const xform& _model2vox )
	{
	...
	// conversion from/to voxel space (continuous) to/from 3d model space (continuous)
	// (where the source point is floating numbers, but target is always snapped to nearest integers)
	static inline void fromVoxToModel( const point& _v, ivec3& _p, const xform& _vox2model )
	{
	...
	static inline void fromModelToVox( const point& _p, ivec3& _v, const xform& _model2vox )
	{
		// convert to voxel space 
	...
	static inline bool outOfVolBounds( const ivec3& _v, const Volume3DScalar* _vol )
	{
		return (
			_v[ 0 ] >= _vol->getSizeX() || _v[ 0 ] < 0 ||
			_v[ 1 ] >= _vol->getSizeY() || _v[ 1 ] < 0 ||
			_v[ 2 ] >= _vol->getSizeZ() || _v[ 2 ] < 0
			);
	}
	static inline bool outOfVolBounds( const ivec3& _v, const shared_ptr<Volume3DScalar>& _vol )
	{
		return outOfVolBounds( _v, _vol.get() );
	}

	// return the value at the given voxel. return 0 if out of bound
	static inline int get_occupancy_at_vox( const ivec3& _v, const Volume3DScalar* _vol )
	{
		auto val = SpaceConverter::outOfVolBounds( _v, _vol ) ?
			0 : _vol->getDataAt( _v[ 0 ], _v[ 1 ], _v[ 2 ] ) > 0.0 ?
			1 : 0;
		return val;
	}
	static inline int get_occupancy_at_vox( const ivec3& _v, const shared_ptr<Volume3DScalar>& _vol )
	{
		return get_occupancy_at_vox( _v, _vol.get() );
	}

	// return the list of 8 nb voxels' coords, given a point in the voxel space
	// Note: the returned voxels are computed using constants defined in Tao's geomcommon.h file
	...
	// return whether the nb 8 voxels of the given point are all inside the volume
	static inline bool allNbVoxelsInside( const point& _p, const shared_ptr<Volume3DScalar>& _vol )
	{
		ivec3 v;
		// locate the given point into a voxel with lowest possible coords
		point p( (int)_p[ 0 ], (int)_p[ 1 ], (int)_p[ 2 ] );
