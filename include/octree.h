...
public:
    ...
    // whether the given node is out side of the space represented by this
    // octree
...
protected: /*helpers*/
    ...
    bool get_node_values(int _i, int _j, int _k, OctreeNode* _node, ivec3 _off,
                         int _len, unsigned char& _vals) const;
...
protected: /*static helpers*/
    // convert source volume into an octree node (covering a region with origin
    // _p and extent _l)
    static OctreeNode* build(const ivec3& _p, int _l,
...
public:
    ...
    struct OctreeNode
    {
        ...
        virtual ~OctreeNode() {}
        OctreeVolume::NodeType getType() const
        {
            return (OctreeVolume::NodeType)type;
        }
    ...
    {
        unsigned char value;
        LeafNode(unsigned char _val) : OctreeNode(NodeType::LEAF), value(_val)
        {
        }
        ...
        // of outputing to file)
        unsigned char getValues() const { return value; };

    protected:
        // return the shift-offset for getting the bit corresponding to the
        // given corner
    ...
    {
        unsigned char value;
        EmptyNode(unsigned char _val) : OctreeNode(NodeType::EMPTY), value(_val)
        {
        }
        bool isHomo(unsigned char& _homo_val) const
        {
    ...
    struct IntNode : public OctreeNode
    {
        ...
        OctreeNode* getChild(int _cidx) const { return children[_cidx]; }
        ...
        void setChild(int _i, OctreeNode* _cp) { children[_i] = _cp; }

    protected:
        // children pointers
        OctreeNode* children[8];
        // return the index for getting the child corresponding to the given
        // octant
    ...
    */
    struct Walker
    {
        ...
        virtual void leaf(OctreeVolume* _tree, OctreeNode* _node,
                          const ivec3& _off, int _len)
        {
        }
        // pre-process an internal node and returns whether pre-condition met or
        // not
        virtual bool pre(OctreeVolume* _tree, OctreeNode* _node,
                         const ivec3& _off, int _len)
        {
            return true;
        }
        // post-process an internal node and returns whether post-condition met
        // or not
        virtual bool post(OctreeVolume* _tree, OctreeNode* _node,
                          const ivec3& _off, int _len)
        {
            return true;
        }
    ...
    void walkSubtree(Walker* _w, OctreeNode* _node, const ivec3& _off,
                     int _len);
};
