...
public:
	...
	SurfacerErrCode extractBoundaryVtsAndFaces( const shared_ptr<Volume3DScalar>& _vol,
	...
	SurfacerErrCode extractBoundaryVts( const shared_ptr<Volume3DScalar>& _vol, vector<point>& _vts );
	// extract boundary for sparse-space-partition structure
	SurfacerErrCode extractBoundaryVtsAndFaces( const shared_ptr<OctreeVolume>& _octree,
		vector<point>& _vts, vector<ivec4>& _faces,
		bool _need_euler );
	// @return the euler characteristic. 
	// @pre face must be 4-tuple of vert indices that trace the boundary if followed in order
	int computeEulerChar( 
		const vector<point>& _vts, const vector<ivec4>& _faces,
...
protected:
	...
	inline void get_corners_coords( int _i, int _j, int _k, const shared_ptr<Volume3DScalar>& _vol, ivec3 * _crns )
	{
		//const auto& mat = _vol->getVoxToModelMat();
		auto mat = trimesh::xform::identity();
		SpaceConverter::fromVoxToModel( point( _i + 0.5f, _j - 0.5f, _k - 0.5f ), _crns[ 0 ], mat );
		SpaceConverter::fromVoxToModel( point( _i + 0.5f, _j + 0.5f, _k - 0.5f ), _crns[ 1 ], mat );
		SpaceConverter::fromVoxToModel( point( _i - 0.5f, _j - 0.5f, _k - 0.5f ), _crns[ 2 ], mat );
		SpaceConverter::fromVoxToModel( point( _i - 0.5f, _j + 0.5f, _k - 0.5f ), _crns[ 3 ], mat );
		SpaceConverter::fromVoxToModel( point( _i + 0.5f, _j - 0.5f, _k + 0.5f ), _crns[ 4 ], mat );
		SpaceConverter::fromVoxToModel( point( _i + 0.5f, _j + 0.5f, _k + 0.5f ), _crns[ 5 ], mat );
		SpaceConverter::fromVoxToModel( point( _i - 0.5f, _j - 0.5f, _k + 0.5f ), _crns[ 6 ], mat );
		SpaceConverter::fromVoxToModel( point( _i - 0.5f, _j + 0.5f, _k + 0.5f ), _crns[ 7 ], mat );
	}
	inline void get_corners_coords( int _i, int _j, int _k, const shared_ptr<Volume3DScalar>& _vol, point* _crns )
	{
		//const auto& mat = _vol->getVoxToModelMat();
		auto mat = trimesh::xform::identity();
		SpaceConverter::fromVoxToModel( point( _i + 0.5f, _j - 0.5f, _k - 0.5f ), _crns[ 0 ], mat );
	...
	inline void get_corners_ids( int _i, int _j, int _k, const shared_ptr<Volume3DScalar>& _vol, ivec3 * _corner_ids )
	{
		_i++; _j++; _k++;
	...
	// each face is represented as 4-tuple of indices into the corner's indices of the same voxel
	inline void get_faces_coords( int _i, int _j, int _k,
		const shared_ptr<Volume3DScalar>& _vol, ivec4 * _faces )
	{
	...
	inline void get_faces_ids( int _i, int _j, int _k,
		const shared_ptr<Volume3DScalar>& _vol, ivec3 * _face_ids )
	{
		_i++; _j++; _k++;
	...
	// main init logic
	void init();
	// init neighbor voxel offset vectors (pointing from cur voxel to its neighbor voxel)
	...
	// init corners & face shared between a voxel and each of the 6 neighboring voxel
	...
	// init for each edge of a voxel cube the offsets of the 4 voxels that share the edge
	// w.r.t. the cube center
	// @note: should only be called by major init() entry
	void set_nbvox_offsets_for_edges();
	// is the corner inside the volume
	inline bool is_corner_inside( const ivec3& _vox, int _c_id, const shared_ptr<Volume3DScalar>& _vol ) const
	{
		point vox( _vox[ 0 ], _vox[ 1 ], _vox[ 2 ] );
		return SpaceConverter::allNbVoxelsInside( vox + SpaceConverter::vox_corner_offset[ _c_id ], _vol );
	}
	// is the edge of the voxel inside the volume
	inline bool is_edge_inside( const ivec3& _vox, int _e_id, const shared_ptr<Volume3DScalar>& _vol ) const
	{
	...
	inline bool is_face_inside( const ivec3& _vox, int _f_id, const shared_ptr<Volume3DScalar>& _vol ) const
	{
	...
	inline bool differ_in_values( const ivec3& _v1, const ivec3& _v2, const shared_ptr<Volume3DScalar>& _vol )
	{
	...
	void save_to_thin_inside_layer( const ivec3& _vox, const shared_ptr<Volume3DScalar>& _vol );
...
protected: /*utility classes*/
	...
	public:
		...
		OctreeLeafCenterCollector(vector<point>& _centers) : m_centers(&_centers) {}
		~OctreeLeafCenterCollector() {}
		// initialize the walker to the proper starting state
		void init();
		// override walker's default behavior
		void leaf( OctreeVolume* _tree, OctreeVolume::OctreeNode* _node, const ivec3& _off, int _len );
	};
