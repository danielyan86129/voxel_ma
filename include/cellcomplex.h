...
struct cellcomplex
{
	...
	*/
	...
	//
	// finalize this cell complex: collecting all 1-cell (edges) from 2-cells (faces), counting references, etc.
	//
	void finalize();
	//
	...
	inline const ivec2& getEdge( size_t _ei ) const
	{
		return m_edges[ _ei ];
	}
	//
	...
	//
	inline void getVertCoords( const vector<int>& _vts_indices, vector<point>& _vts_coords ) const
	{
		_vts_coords.clear();
	...
	//
	inline const vector<point>& getAllVts() const
	{
		return m_vts;
	}
	//
	...
	//
	inline int refCount( size_t _ei ) const
	{
		return m_edge_ref_cnt[ _ei ];
	}
	//
	// num of faces
	//
	...
	{
		return m_faceIndex.size();
	}
	//
	// num of vertices
	//
	inline size_t numVts() const
	{
		return m_vts.size();
	}
	// 
	...
	//
	void appendFace( const vector<int>& _f_of_vts, const vector<int>& _f_of_sides );
	// 
	// add an edge (could be referenced by a face as part of its boundary, or an isolated edge)
	//
	inline void appendEdge( const ivec2& _e )
	{
	...
	// add a vertex
	//
	inline void appendVert( const point& _v )
	{
		m_vts.push_back( _v );
	}
	//
