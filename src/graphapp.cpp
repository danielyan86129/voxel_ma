...
namespace graphapp
{
	bool graphapp::readGraph( const string & _filename,
		...
		vector<vector<float>>& _msures )
	{
		...
		_edges.clear(); _edges.reserve( nEdges );
		_msures.resize( 3 );
		auto& bt3 = _msures[ 0 ];
		auto& bt2 = _msures[ 1 ];
		auto& bt1 = _msures[ 2 ];
		...
		vector<float>& _weights )
	{
		...
		case graphapp::BT3:
		{
			...
			auto min_max = std::minmax_element( _weights.begin(), _weights.end() );
		...
		if (!indexable_by_outside_idx || !matched )
		{
			if ( !indexable_by_outside_idx )
			{
				cout << "WARNING: sanity checking failed. "
					<< "outside index doesn't correspond to 1st arg in boost::vertex()!" << endl;
			}
			if ( !matched )
			{
				cout << "WARNING: sanity checking failed. "
					<< "index from index-map and that stored within vertex doesn't match!" << endl;
			}
			return false;
		}
		return true;
		...
		NodeHandle& _root )
	{
		...
		for ( boost::tie( e_it, e_end_it ) = boost::edges( _g );
			e_it != e_end_it; ++e_it )
		{
		...
		float max_r = 0.0f;
		for ( boost::tie( vit, vit_end ) = boost::vertices( _g );
			vit != vit_end; ++vit )
		{
		...
		vector<NodeHandle>& _t )
	{
		...
		{
			cout << "Done: vertex " << _g[ _root ].idx << ", " << _g[ _root ].p << " is root." << endl;
		}
		...
		vector<NodeHandle>& _parent )
	{
		try
		{
			...
			boost::dijkstra_shortest_paths( _g, _s,
				boost::weight_map( e_wt_map ).
				predecessor_map( boost::make_iterator_property_map( _parent.begin(), v_id_map ) ).
				distance_map( boost::make_iterator_property_map( dist.begin(), v_id_map ) )
			);
	...
	void exportTree( const WeightedGraph & _g, vector<NodeHandle>& _t, const std::string & _filename )
	{
