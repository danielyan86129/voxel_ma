	...
	bool _need_euler)
{
	...
	auto process_voxel = [&] (int i, int j, int k)
	{
		...
		for ( auto o = 0; o < m_faceId_nbvoxOffset_map.size(); ++o )
		{
			...
			// debug begin
			auto cur_vox_val = SpaceConverter::get_occupancy_at_vox( cur_vox, _vol );
			auto nb_vox_val = SpaceConverter::get_occupancy_at_vox( nb_vox, _vol );
			...
			if ( differ_in_values( cur_vox, nb_vox, _vol ) )
			{
				...
				// for each corner between curvox and nbvox, test whether it exists or not
				for ( auto ii = 0; ii < one_side_crnrs.size(); ++ii )
				{
					...
					{
						// this corner wasn't created before. add it to vts_list.
						const auto& c = cnrs_coords[ ci ];
						_vts.push_back( c );
						//_vts.push_back( ivec3( c[ 0 ], c[ 1 ], c[ 2 ] ) );
						access_elem_index[ cnrs_ids[ ci ] ] = (int)( _vts.size() - 1 );
					}
				}
				// test whether the striding face exists or not
				if ( access_elem_index.find( faces_ids[ fi ] ) == access_elem_index.end() )
				{
	...
	{ // process only boundary voxels since we are dealing with octree
		vector<ivec3> boundary_voxels;
		cout << "getting boundary voxels..." << endl;
		vol_ptr->getBoundaryVoxels( boundary_voxels );
		cout << "# boundary voxels obtained: " << boundary_voxels.size() << endl;
...
{
	vector<ivec4> quads;
	auto errcode = extractBoundaryVtsAndFaces( _vol, _vts, quads, _euler != nullptr );
...
SurfacerErrCode Surfacer::extractBoundaryVts( const shared_ptr<Volume3DScalar>& _vol, vector<point>& _vts )
{
	if ( !_vol )
		return SurfacerErrCode::EMPTY_VOL;
	_vts.clear();
	...
	eulerchar& _euler) const
{
	...
	auto multiplicity_v = [&]( int _vi ) -> int {
		...
		point q;
		ivec3 nb_voxels[ 8 ];
		SpaceConverter::fromModelToVox( p, q, /*_vol->getModeltoVoxMat()*/trimesh::xform::identity() );
		SpaceConverter::getNbVoxelCoords( q, nb_voxels );
		unsigned char mask = 0;
		for ( int i = 0; i < 8; ++i )
		{
			mask |= ( SpaceConverter::get_occupancy_at_vox( nb_voxels[ i ], _vol ) << (7 - i) );
		}
	...
	E_int /= 4;
	F_int /= 2;
	std::cout << "# interior V / E / F = " << V_int << " / " << E_int << " / " << F_int << std::endl;

	// now we need to figure out the last part of Euler Char: the num of connected components
...
void Surfacer::set_cube_cmpnts_num()
{
	...
	for ( unsigned char mask = 0; mask <= 255; ++mask )
	{
		...
		// initially, each mask config corresponds to at least 1 surface component
		// and no edge is ref.ed by any cycle yet
		m_nCmpnts_per_cube[ mask ] = 0;
...
void Surfacer::set_nbvox_offsets_for_edges()
{
	...
	// possible config of ijk. Each seq. specifies which one of ijk should stay 0, 
	// followed by the other 2 as changing iterators
...
void Surfacer::save_to_thin_inside_layer( const ivec3 & _vox, const shared_ptr<Volume3DScalar>& _vol )
{
	...
	for ( int i = -1; i <= 1; ++i )
	{
		for ( int j = -1; j <= 1; ++j )
		{
			...
			{
				nb = _vox + ivec3( i, j, k );
				if ( SpaceConverter::get_occupancy_at_vox( _vox + ivec3( i, j, k ), _vol ) == 1 )
				{
	...
	for ( const auto& v : m_inside_voxs_thin )
	{
		...
		while (!cmpnt_q.empty() )
		{
			...
			if (visited.count(cur_v) > 0)
				continue;
			// else, set it as visited, and expand the unvisited & inside neighbors of cur_v into the queue
			...
			for ( const auto& o : m_faceId_nbvoxOffset_map )
			{
				...
				// then visit it
				if ( m_inside_voxs_thin.count( nb ) > 0 &&
					visited.count( nb ) == 0 )
				{
					cmpnt_q.push( nb );
				}
...
void Surfacer::OctreeLeafCenterCollector::init()
{
	m_centers->clear();
}

void Surfacer::OctreeLeafCenterCollector::leaf(
	OctreeVolume * _tree, OctreeVolume::OctreeNode * _node, 
