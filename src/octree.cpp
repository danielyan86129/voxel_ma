...
OctreeVolume::OctreeVolume( const Volume3DScalar * _src_vol )
{
	...
	m_max_res = std::max( 
		std::max( _src_vol->getSizeX(), _src_vol->getSizeY() ), 
		_src_vol->getSizeZ() ) - 1;
	m_max_res = 1 << util::log2Int( m_max_res );
	cout << "m_max_res = " << m_max_res << endl;
	// size of voxels (each sitting at corners of a node)
	Volume3DScalar::m_sizes = ( _src_vol->getSizeX(), _src_vol->getSizeY(), _src_vol->getSizeZ() );
...
unsigned char OctreeVolume::getNodeValues( int _i, int _j, int _k ) const
{
	...
	{
		auto data = m_data;
		auto retcode = get_node_values( _i, _j, _k, m_root, ivec3( 0, 0, 0 ), m_max_res, val );
		assert( retcode == true );
	}
	return val;
}
...
void OctreeVolume::getBoundaryVoxels( vector<ivec3>& _voxels ) const
{
	...
	cout << "# leaves with all 0s/1s: " << all_zero_leaf_cnt << "/" << all_one_leaf_cnt << endl;
...
bool OctreeVolume::read_sof_file( const string & _sof_file )
{
		...
		[ & ]( const ivec3& _off, int _len ) -> OctreeNode*
	{
		...
		else
		{
			...
			cout << "octree so far constructed - Leaf/empty/internal: "
				<< leaf_cnt << "/" << empty_cnt << "/" << internal_cnt << endl << endl;
			exit( -1 );
		}
	};
	...
	cout << "octree constructed. Leaf/empty/internal: "
		<< leaf_cnt << "/" << empty_cnt << "/" << internal_cnt << endl << endl;
...
bool OctreeVolume::read_sog_file( const string & _sog_file )
{
	...
	float x, y, z;
	x = *(float*)(buf); buf += sizeof( float );
	y = *(float*)(buf); buf += sizeof( float );
	z = *(float*)(buf); buf += sizeof( float );
	auto lowerleft = point( x, y, z );
	float bbox_len_input_space = *(float*)( buf ); buf += sizeof( float );
	delete[] header_buf;
	...
	//cout << "vox to mesh space transformation: " << Volume3DScalar::m_vox_to_mesh << endl;
	...
	for ( auto i = 0; i < auto_mesh_to_vox.size(); ++i )
	{
		...
		{
			cout << "Warning: mesh_to_vox and auto_vox_to_mesh are not the same!" << endl;
...
bool OctreeVolume::get_node_values(
	...
	unsigned char& _vals ) const
{
	...
	if (
		ncoord_t[ 0 ] < 0 || ncoord_t[ 0 ] >= _len ||
		ncoord_t[ 1 ] < 0 || ncoord_t[ 1 ] >= _len ||
		ncoord_t[ 2 ] < 0 || ncoord_t[ 2 ] >= _len
		)
	{
		return false;
	}
	...
	if ( type == 2 )
	{
		...
		{
			cout << "Assertion off == (i,j,k) failed! "
				<< _off << ", " << ivec3( _i, _j, _k ) << endl;
			exit(-1);
		}
		// leaf node. no need to recurse.
		auto leafnode = dynamic_cast<LeafNode*>( _node );
		...
		/*if ( ret_values == 0 )
			...
			cout << "(boundary) leaf node value: " << (int)ret_values << endl;*/
	}
	else if ( type == 1 )
	{
		...
		for ( int i = 0; i < 2; i++ )
			for ( int j = 0; j < 2; j++ )
				for ( int k = 0; k < 2; k++ )
				{
					...
					auto childnode = intnode->getChild( /*k, j, i*/i, j, k );
					found = get_node_values( _i, _j, _k, childnode, node_off, node_len, node_values );
					if ( found )
					{
		...
		for ( int i = 0; i < 2; ++i )
			for ( int j = 0; j < 2; ++j )
				for ( int k = 0; k < 2; ++k )
				{
					...
					if ( value1 != value2 )
					{
						cout << "values doesn't match! At node {"
							<< (int)type << ", " << _off << ", " << _len << "}, iter "
							<< ivec3( i, j, k ) 
		...
		for ( int i = 0; i < 2; ++i )
			for ( int j = 0; j < 2; ++j )
				for ( int k = 0; k < 2; ++k )
				{
					...
					auto node_ret = test_node( intnode->getChild( /*k, j, i*/i, j, k ), noff, nlen );
					if ( node_ret == false )
					{
						ret_code = false;
					}
...
OctreeVolume::OctreeNode* OctreeVolume::build( const ivec3& _p, int _l, const Volume3DScalar* _src_vol )
{
	/* region (p, l) is outside of _src_vol completely*/
	if ( !util::intersect( 
		_p, { _l, _l, _l },
		{ 0,0,0 }, { _src_vol->getSizeX(), _src_vol->getSizeY(), _src_vol->getSizeZ() } ) 
		)
	{
	...
	else
	{
		...
		if ( _l == 1 ) // bottom octree node. collect values of leaves below it.
		{
			...
			for ( auto i = 0; i < 2; ++i ) for ( auto j = 0; j < 2; ++j ) for ( auto k = 0; k < 2; ++k )
			{
				auto off = ivec3( /*k, j, i*/i, j, k );
				auto vox_val = SpaceConverter::get_occupancy_at_vox( _p + off, _src_vol );
				vox_val = vox_val == 1 ? 0 : 1;
				//if (vox_val != 0) cout << "leaf corner value: " << vox_val << endl;
...
void OctreeVolume::walkSubtree( Walker * _w, OctreeNode * _node, const ivec3 & _off, int _len )
{
