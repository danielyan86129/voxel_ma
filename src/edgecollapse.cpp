...
namespace voxelvoro
{
	...
	void TopoPreservEdgeCollapser::preProcess()
	{
		compute_order();
	}
	...
	void TopoPreservEdgeCollapser::collapse()
	{
		...
		std::cout << "total # edges to inspect: " << e_f_adj_tbl.size() << std::endl;
		...
		for ( auto it = e_f_adj_tbl.begin(); it != e_f_adj_tbl.end(); /*++it*/ )
		{
			...
			// find the size of the intersected sets, compare the size with that of the edge uv
			lk_u_vts.clear();
			lk_u_vts.insert( nb_vts_u.begin(), nb_vts_u.end() );
			int num_common_vts = std::count_if( nb_vts_v.begin(), nb_vts_v.end(), [&](int _vid) {
			...
			// check if this holds for the link for u, v containing edges (nb edges):
			// reject edge if the intersection between lk_u and lk_v is non-empty 
			// (as lk_uv contains only vertices on 2-complex)
			lk_u_edges.clear();
			...
			{
				// also account for the common edges with dummy vertex as one of the ends
				// for u, v find the edges involving dummy vertex in their resp. lk
				// and see if there are any in common
				// find such edges for u first
				for ( const auto& nb_vi : nb_vts_u )
				{
					nb_e = util::makeEdge( u, nb_vi );
					// if nb edge is singular, a dummy vertex is connected to the nb_vi
					// to form an oppo_e for vertex u. mark this oppo-e as a potential common edge
					// between lk_u and lk_v
				...
				for ( const auto& nb_vi : nb_vts_v )
				{
					...
					// if nb edge is singular, a dummy vertex is connected to the nb_vi
					// to form an oppo_e for vertex v. check if lk_u has any edge in common
					if ( e_f_adj_tbl.find( nb_e )->second.size() != 2 )
					{
			...
			{
				nb_e = util::makeEdge( v, nb_vi );
				// if nb edge is singular, check the nb to see if it's common in lk_1_u
				if ( e_f_adj_tbl.find( nb_e )->second.size() != 2 )
				{
			...
			collapse the edge. merge/update the affected vts/edges' neighborhoods properly
			including the v-v-adj, v-f-adj, and e-v-adj */
			...
			for ( const auto& fi_to_remove : lk_uv )
			{
				...
				if ( e_us_it == e_f_adj_tbl.end() )
				{
					std::cout << "Error (during edge u-s lookup): edge " 
						<< e_us_it->first << " is not in e-f-adj-tbl." << std::endl;
					exit( -1 );
				}
				...
				if ( e_vs_it == e_f_adj_tbl.end() )
				{
					std::cout << "Error (during edge v-s lookup): edge " 
						<< e_vs_it->first << " is not in e-f-adj-tbl." << std::endl;
					exit( -1 );
				}
				...
				for ( auto it = nbvts_s_to_modify.begin(); it != nbvts_s_to_modify.end();
					++it )
				{
					if ( *it == u )
					{
			...
			nbvts_v_to_modify.insert( nbvts_v_to_modify.end(), vts_of_merged_nb.begin(), vts_of_merged_nb.end() );
				...
				if ( nbvi != v )
				{
					...
					if ( old_e_it == e_f_adj_tbl.end() )
					{
						std::cout << "Error (during edge u_nb-u lookup): edge " 
							<< old_e_it->first << " is not in e-f-adj-tbl." << std::endl;
						exit( -1 );
					}
			...
			*/
			...
			for ( auto i = 0; i < vts.size(); ++i )
			{
				...
				while ( v_remain_deg[ start_v ] > 0 )
				{
					...
					{
						topo_v_v_adj_tbl[ topo_e[ 0 ] ].push_back( ivec2( topo_e[ 1 ], topo_edges.size() - 1 ) );
						topo_v_v_adj_tbl[ topo_e[ 1 ] ].push_back( ivec2( topo_e[ 0 ], topo_edges.size() - 1 ) );
					}
				}
			}
			...
			{
				bool has_cycle = false; // including loop, but can be higher order cycle
				...
				if (!has_cycle )
				{
					...
					// Since we know at this point there is no loop topo edge in the graph
					// and all topo vts are non-manifold vts
					// so we can detect cycle easily locally at each topo edge:
					// we set the ends of a topo edge as visited after inspecting that edge
					// if at some point we find both ends of an edge are "visited", then there is a cycle.
				...
				else
				{
					...
					else // it's a junction point that has most complicated neighborhood
					{
