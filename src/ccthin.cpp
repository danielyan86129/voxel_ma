...
void CellComplexThinning::setup( cellcomplex* _cc )
{
	m_cc = _cc;
}
...
void CellComplexThinning::reset()
{
	...
	{
		rmlist.clear(); rmlist.shrink_to_fit();
	}
...
void CellComplexThinning::assignElementValues(
	const vector<float>& _vert_measures,
	...
	)
{
	...
	min_f_measure = numeric_limits<float>::max(); max_f_measure = numeric_limits<float>::min();
	min_e_measure = min_f_measure; max_e_measure = max_f_measure;
}
...
void CellComplexThinning::prune( float _f_t, float _l_t, bool _remove_small_components )
{
	auto sanity_check = [&]()
	{
	...
	*/
	...
	//		{
	//			unsigned nb_f;
	//			for ( auto nb_f_i = 0; nb_f_i < m_cc->cntNbFacesofEdge( ei ); ++nb_f_i )
	//			{
	...
	for (unsigned fi = 0; fi < m_cc->numFaces(); ++fi)
	{
		...
		{
			if ( m_ref_edge_per_prune[ei] == 1 && 
				!m_removed[EDGE][ei] && !m_removed[FACE][fi] &&
			...
			for ( auto ei : f )
			{
				if ( !m_removed[ EDGE ][ ei ] )
				{
					...
					for ( auto nb_f_i = 0; nb_f_i < m_cc->cntNbFacesofEdge( ei ); ++nb_f_i )
					{
						...
						if ( !m_removed[ FACE ][ nb_f ] )
							cout << nb_f << ( m_to_remove_face[ nb_f ] ? "(m)" : "" ) << ", ";
					}
					cout << endl;
				}
			}
...
bool CellComplexThinning::face_edge_pair_below_threshold(
	unsigned _fi, unsigned _ei, float _f_t, float _e_t ) const
{
...
void CellComplexThinning::prune_while_iteration(
	const set<unsigned>& vts_to_debug, float _f_t, float _l_t, std::queue<simple_pair>& q
	)
{
	...
	while ( !q.empty() )
	{
		...
		if ( spr.type == simple_pair::FE_PAIR && spr.idx1 == ei_debug )
		{
			cout << "got face-edge spr (" << spr.idx0 << "," << spr.idx1 << ")"
				<< " face removed?" << ( m_removed[ FACE ][ spr.idx0 ] ? "true" : "false" ) << endl;
			print_edge( ei_debug );
		}
		...
		// logic checkpoint
		if (
			!( spr.type == simple_pair::FE_PAIR && m_ref_edge_per_prune[ spr.idx1 ] == 1 ||
				spr.type == simple_pair::EV_PAIR && m_ref_vert_per_prune[ spr.idx1 ] == 1 )
			)
		{
		...
		if ( spr.type == simple_pair::EV_PAIR ) // edge-vert pair
		{
			...
			{
				for ( auto nb_e_i = 0; nb_e_i < m_cc->cntNbEdgesofVert(other_end); ++nb_e_i )
				{
					...
					if ( !m_removed[ EDGE ][ nb_e ] )
					{
						if ( edge_vert_pair_below_threshold( nb_e, _l_t ) )
						{
							q.push( simple_pair( simple_pair::EV_PAIR, nb_e, other_end ) );

							if ( vts_to_debug.count( other_end ) ) //debug
							{
								cout << "simple pair added: ("
									<< 0 << ", " << nb_e << ", " << other_end << ")" << endl;
							}
						}
						break;
					}
		...
		else // face-edge pair
		{
			...
			for ( unsigned i = 0; i < 2; ++i )
			{
				...
				if ( m_ref_vert_per_prune[ vi ] == 1 )
				{
					...
					for ( auto ith_nb_e = 0; ith_nb_e < m_cc->cntNbEdgesofVert( vi ); ++ith_nb_e )
					{
						...
						if ( !m_removed[ EDGE ][ nb_ei ] )
						{
							...
							{
								q.push( simple_pair( simple_pair::EV_PAIR, nb_ei, vi ) );
...
void CellComplexThinning::mark_components( float _cmpnt_geomSize_t, int _num_face_t )
{
	...
	*/
	...
	cout << cnt_small_cmpnts << " small components found (marked as to-remove). "<<endl;
