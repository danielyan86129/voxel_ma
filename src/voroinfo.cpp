...
namespace voxelvoro {
	...
	bool VoroInfo::loadFromTetgenFiles( const char * _file_basename, shared_ptr<Volume3DScalar> _vol )
	{
		...
		*/
		...
		cout << "time(I/O) -> load voro from tetgen files: " << t_IO.elapseMilli().count() << " ms" << endl;
	...
	bool VoroInfo::loadFromTetgenFiles( const tetgenio & _tetio, shared_ptr<Volume3DScalar> _vol )
	{
		timer t;
		invalidate_geometric_states();
		...
		cout << "time -> load voro from tetgen: " << t.elapseMilli().count() << " ms" << endl;
		return true;
	}
