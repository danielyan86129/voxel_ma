...
namespace voxelvoro {
	...
	void VoroInfo::computeInfoRelatedtoSites( const vector<point>& _cell_cents )
	{
		...
		for ( auto ci = 0; ci < _cell_cents.size(); ++ci )
		{
			...
			query_p[ 1 ] = cent[ 1 ];
			query_p[ 2 ] = cent[ 2 ];
			/*auto nn_within_r = kdtree->annkFRSearch( query_p, sq_range, 1, nn_idx, nullptr, 0.0 );
			if ( nn_within_r == 0 )
			cout << "WARNING: cell centroid "<<ci<<" has zero closest site. Impossible!" << endl;
			else
		...
		//		auto nn_within_r = kdtree->annkFRSearch( query_p, sq_range, 1, nn_idx, nullptr, 0.0 );
		//		if ( nn_within_r == 0 )
		//			cout << "WARNING: vert " << vi << ", zero NN found!" << endl;
		//		else
		...
		// Solution 2: from the sites indices of each face derive the sites for each vertex
	...
	bool VoroInfo::radiiValid() const
	{
		return m_r_valid;
	}

	const vector<float>& VoroInfo::getRadii() const
	{
		return m_r_per_v;
	}
	...
	bool VoroInfo::tagVtsUsingUniformVol( const shared_ptr<Volume3DScalar>& _vol )
	{
		...
		for ( size_t i = 0; i < m_geom.numVts(); ++i )
		{
			...
			SpaceConverter::fromModelToVox( pt, vox, /*_vol->getModeltoVoxMat()*/trimesh::xform::identity() );
	...
	void VoroInfo::getVts( const vector<int>& _vts_indices, vector<point>& _vts ) const
	{
	...
	void VoroInfo::getFaces( const vector<int>& _faces_indices, vector<uTriFace>& _tri_faces ) const
	{
		vector<int> f_vrep;
	...
	void VoroInfo::getPartInside( vector<int>& _vts_indices, vector<ivec2>& _edges, vector<uTriFace>& _tri_faces ) const
	{
		// prepare vertices:
		// - compact away invalid vertices
		// - maintain a mapping: old vertex index -> new index in the list of remaining vts
		...
		vector<float>& _vts_msure, vector<float>& _edges_msure, vector<float>& _faces_msure ) const
	{
		...
		cout << "time -> extract inside: " << t_get_inside.elapseMilli().count() << " ms" << endl;
		cout << "time -> compute measure: " << t_msure.elapseMilli().count() << " ms" << endl;
	}
	...
	//	// for vertex: before merging index -> after merging index (into the set of remaining vts)
	//	vector<int> old_to_new_idx;
	//	// the remaining vertices after merging (indices into the old vts list)
	//	vector<int> new_vts_indices;
	//	// the set of remaining edges which might be important to preserving topology after merging
	...
	//		// obtain all nearest neighbors within the ball of the specified radius
	//		temp_timer.start();
	//		nn_within_r = kdtree->annkFRSearch( query_p, sq_r, 0, nullptr, nullptr, 0.0 );
	//		if ( nn_within_r == 0 )
	//			cout << "WARNING: vert "<<vi << ", zero NN found!" << endl;
	//		nn_idx = new ANNidx[ nn_within_r ];
	//		kdtree->annkFRSearch( query_p, sq_r, nn_within_r, nn_idx, nullptr, 0.0 );
	//		temp_timer.stop();
	//		search_knn_t += temp_timer.elapse();
	//		
	...
	//	// to those faces may need to be merged to near-by faces' attributes list
	...
	//	}
	//	//cout << endl;
	//	cout << "done: checking and handling faces degeneracy after merging." << endl;
	...
	//		<< duration_cast<milliseconds>( build_knn_t ).count() << "ms" << endl;
	//	cout << "knn search: " 
	//		<< duration_cast<milliseconds>(search_knn_t).count() << "ms" << endl;
	//	cout << "merging operation: " 
	...
	void VoroInfo::mergeCloseVts( float _eps, bool _only_inside )
	{
		...
		for ( size_t vi = 0; vi < n_pts; ++vi )
		{
			...
			vector<int> nb_vs; nb_vs.reserve( 50 );
			while ( !close_q.empty() )
			{
	...
	void VoroInfo::computeEulerChar( eulerchar & _euler_struct, bool _do_prune_first )
	{
		...
		if ( _do_prune_first )
		{
			...
			C = compute_conn_cmpnts(&valid_faces_indices, 1);
		}
		else
		{
			V = (int)m_geom.numVts();
	...
	void VoroInfo::outputToMathematica( const char * _filename ) const
	{
		...
		cout << "Writing current voro geometry (2-complex) to Mathematica..." << endl;
		...
		ofile.seekp( -1, ofile.cur );
		ofile << "}" << "(* end: faces *)" << endl;

		// output outmost }
		ofile << '}' << endl;
		ofile.close();

		cout << "Done: Writing current voro geometry (2-complex) to Mathematica." << endl;
	}
	...
	void VoroInfo::infer_sites_from_cell_file( ifstream & _in_cells )
	{
		...
		for ( auto ci = 0; ci < cell_cnt; ++ci )
		{
			...
			{
				std::cout << "WARNING: couldn't estimate cell centroid for cell " << ci << ". "
	...
	void VoroInfo::trace_face( const vector<int>& _f_of_sides, const vector<ivec2>& _edges, vector<int>& _f_of_vts ) const
	{
		ivec2 e1, e2;
	...
	int VoroInfo::compute_conn_cmpnts(const vector<int>* _subset_faces)
	{
		int ncc = 0;
		// construct 
		...
		vector<bool> visited_f( m_geom.numFaces(), false );
		vector<bool> visited_e( m_geom.numEdges(), false );
		vector<int> f_e_rep;
		if ( _subset_faces )
		{
		...
		return ncc;
	} // VoroInfo::compute_conn_cmpnts();

	int VoroInfo::compute_conn_cmpnts( const vector<int>* _subset_faces, int _flood_across_vertex )
	{
		int ncc = 0;
		// construct 
		// - vert_id-vert_id adjacency table
		vector< vector<int> > v_v_adj_tbl;
		v_v_adj_tbl.resize( m_geom.numVts() );
		vector<bool> visited( m_geom.numVts(), false );
		vector<int> f_v_rep;
		vector<int> f_e_rep;
		if ( _subset_faces )
		{
		...
		else
		{
			...
			// build v-v-adj table
			for ( const auto& e : m_geom.m_edges )
			{
				v_v_adj_tbl[ e[ 0 ] ].push_back( e[ 1 ] );
				v_v_adj_tbl[ e[ 1 ] ].push_back( e[ 0 ] );
			}
		...
		ncc = util::findNumConnComponents( v_v_adj_tbl, m_geom.numVts(), visited );
		///*begin flooding*/
		//queue<int> vts_q;
		//for ( int vi = 0; vi < m_vts.size(); ++vi )
		//{
		//	if (visited[vi] )
		//		continue;
